...
### 2\. Reward Model (`distilgpt2-sequence-classification`)

  * **Source:** Initialized from `distilgpt2` weights with a custom `SequenceClassification` head.
  * **Parameters:** \~82 Million + 1 Linear Layer ($768 \times 1$).
...
Results are stored in `runs/ppo_distilgpt2/`:
...
## Known Limitations
  ...
  * **VRAM Constraints:** Limited to small-scale models (DistilGPT-2) to fit all three models (Policy, Ref, Reward) on a single 12GB GPU.
